...
def binplotter(ax1, nrOfVars, operationStr, vtree, heuristic, nrOfClauses, indexCounter, colors, noOverhead = False):
    filename = f"output/heuristic/test_20_{nrOfVars}_{operationStr}_{vtree}_{[heuristic]}_{nrOfClauses}.txt"
    if noOverhead:
        filename = f"output/heuristic/noOverhead_20_{nrOfVars}_{operationStr}_{vtree}_{[heuristic]}_{nrOfClauses}.txt"
    lines = []
    try:
        with open(filename, 'r') as file:
            lines = file.readlines()
...
def boxplotter(ax1, nrOfVars, operationStr, vtree, heuristic, nrOfClauses, indexCounter, colors, noOverhead = False):
    filename = f"output/heuristic/test_20_{nrOfVars}_{operationStr}_{vtree}_{[heuristic]}_{nrOfClauses}.txt"
    if noOverhead:
        filename = f"output/heuristic/noOverhead_20_{nrOfVars}_{operationStr}_{vtree}_{[heuristic]}_{nrOfClauses}.txt"
    lines = []
    ...
    for index, line in enumerate(lines):
        try:
            ...
            # for whisker in box['whiskers']: #verticale lijn
            #     whisker.set_color('red') 
            # for cap in box['caps']: #horizontale uiteinden
            #     cap.set_color('blue')
            for median in box['medians']: #gemiddelde
                median.set_color('black')
            # for flier in box['fliers']: #uitschieters
...
def heuristicsPlot(heuristics, nrOfVars, vtree, operation):
    ...
    for nrOfClauses in nrOfClausesLists:
        ...
        for heuristic in heuristics:
            ...
            boxplotter(ax1, nrOfVars, operationStr, vtree, heuristic, nrOfClauses, indexCounter, colors)
            # binplotter(ax1, nrOfVars, operationStr, vtree, heuristic, nrOfClauses, indexCounter, colors)
            indexCounter += 1
        boxplotter(ax1, nrOfVars, operationStr, vtree, VP_EL, nrOfClauses, indexCounter, colors, noOverhead = True)
...
def plotter(metric, nrOfVars, operationStr, vtree, heuristic, nrOfClauses, indexCounter, colors):
...
def otherMetricsPlot(heuristics, nrOfVars, vtree, operation):
    ...
    for (i, metric) in enumerate(metrics):
        for nrOfClauses in range(int(nrOfVars*0.5), nrOfVars*5, int(nrOfVars*0.5)):
            indexCounter = 0
            for heuristic in heuristics:
                plotter(metric, nrOfVars, operationStr, vtree, heuristic, nrOfClauses, indexCounter, colors)
                indexCounter += 1
